...
import {
  monetizationAdProviderCreateApi,
  monetizationAdProviderPageApi,
  monetizationAdProviderUpdateApi,
  monetizationAdProviderUpdateStatusApi,
} from '#/api/core';
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page, sorts }, formValues?: AdProviderSearchValues) =>
        await monetizationAdProviderPageApi(
          formatQuery({
            page,
...
function buildAdProviderSearchValues(formValues: AdProviderSearchValues = {}) {
  ...
  } satisfies Partial<MonetizationAdProviderPageRequest>;
}
...
async function handleCreateSubmit(values: AdProviderFormValues) {
  await monetizationAdProviderCreateApi(buildAdProviderCreatePayload(values));
  useMessage.success('操作成功');
  await adProviderGridApi.reload();
}

async function handleEditSubmit(values: AdProviderFormValues) {
  await monetizationAdProviderUpdateApi(buildAdProviderUpdatePayload(values));
  useMessage.success('操作成功');
  await adProviderGridApi.reload();
}
...
async function toggleEnableStatus(row: AdProviderRow) {
  ...
  try {
    await monetizationAdProviderUpdateStatusApi({
      id: row.id,
      isEnabled: row.isEnabled !== true,
    } satisfies MonetizationAdProviderUpdateStatusRequest);
