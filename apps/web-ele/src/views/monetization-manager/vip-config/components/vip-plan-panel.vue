...
import {
  monetizationVipPlanCreateApi,
  monetizationVipPlanPageApi,
  monetizationVipPlanUpdateApi,
  monetizationVipPlanUpdateStatusApi,
} from '#/api/core';
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page, sorts }, formValues) => {
        return await monetizationVipPlanPageApi(
          formatQuery({
...
function buildVipPlanSearchValues(formValues: VipPlanSearchValues = {}) {
  ...
  } satisfies Partial<MonetizationVipPlanPageRequest>;
}
...
async function handleSubmit(values: VipPlanFormValues) {
  await (values.id
    ? monetizationVipPlanUpdateApi(buildVipPlanUpdatePayload(values))
    : monetizationVipPlanCreateApi(buildVipPlanCreatePayload(values)));
...
async function toggleEnableStatus(row: VipPlanRow) {
  ...
  try {
    await monetizationVipPlanUpdateStatusApi({
      id: row.id,
      isEnabled: !row.isEnabled,
    } satisfies MonetizationVipPlanUpdateStatusRequest);
