...
import {
  monetizationVipPageConfigCreateApi,
  monetizationVipPageConfigPageApi,
  monetizationVipPageConfigUpdateApi,
  monetizationVipPageConfigUpdateStatusApi,
} from '#/api/core';
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page, sorts }, formValues?: VipPageConfigSearchValues) =>
        await monetizationVipPageConfigPageApi(
          formatQuery({
            page,
...
function buildVipPageConfigSearchValues(
  ...
  } satisfies Partial<MonetizationVipPageConfigPageRequest>;
}
...
async function handleCreateSubmit(values: VipPageConfigFormValues) {
  await monetizationVipPageConfigCreateApi(
    buildVipPageConfigCreatePayload(values),
  );
...
async function handleEditSubmit(values: VipPageConfigFormValues) {
  await monetizationVipPageConfigUpdateApi(
    buildVipPageConfigUpdatePayload(values),
  );
  useMessage.success('操作成功');
  await vipPageConfigGridApi.reload();
}
...
async function toggleEnableStatus(row: VipPageConfigRow) {
  ...
  try {
    await monetizationVipPageConfigUpdateStatusApi({
      id: row.id,
      isEnabled: row.isEnabled !== true,
    } satisfies MonetizationVipPageConfigUpdateStatusRequest);
