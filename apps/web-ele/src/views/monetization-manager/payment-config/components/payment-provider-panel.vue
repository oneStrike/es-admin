...
import {
  monetizationPaymentProviderCreateApi,
  monetizationPaymentProviderPageApi,
  monetizationPaymentProviderUpdateApi,
  monetizationPaymentProviderUpdateStatusApi,
} from '#/api/core';
  ...
  proxyConfig: {
    ajax: {
      query: async (
        ...
        await monetizationPaymentProviderPageApi(
          formatQuery({
            page,
...
function buildPaymentProviderSearchValues(
  ...
  return {
    ...
    startDate,
  } satisfies Partial<MonetizationPaymentProviderPageRequest>;
}
...
async function handleCreateSubmit(values: PaymentProviderFormValues) {
  await monetizationPaymentProviderCreateApi(
    buildPaymentProviderCreatePayload(values),
  );
...
async function handleEditSubmit(values: PaymentProviderFormValues) {
  await monetizationPaymentProviderUpdateApi(
    buildPaymentProviderUpdatePayload(values),
  );
  useMessage.success('操作成功');
  await paymentProviderGridApi.reload();
}
...
async function toggleEnableStatus(row: PaymentProviderConfigRow) {
  ...
  try {
    await monetizationPaymentProviderUpdateStatusApi({
      id: row.id,
      isEnabled: row.isEnabled !== true,
    } satisfies MonetizationPaymentProviderUpdateStatusRequest);
