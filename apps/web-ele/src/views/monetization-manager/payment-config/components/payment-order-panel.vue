...
import type { MonetizationPaymentOrderPageRequest } from '#/api/types';

import { useVbenModal } from '@vben/common-ui';

import { formatQuery, useVbenVxeGrid } from '#/adapter/vxe-table';
import {
  monetizationPaymentOrderPageApi,
  monetizationPaymentOrderUpdateStatusApi,
} from '#/api/core';
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page, sorts }, formValues) => {
        return await monetizationPaymentOrderPageApi(
          formatQuery({
...
function buildPaymentOrderSearchValues(
  ...
  } satisfies Partial<MonetizationPaymentOrderPageRequest>;
}
...
async function handleConfirm(values: PaymentOrderConfirmFormValues) {
  try {
    await monetizationPaymentOrderUpdateStatusApi(
      buildPaymentOrderConfirmPayload(values),
    );
