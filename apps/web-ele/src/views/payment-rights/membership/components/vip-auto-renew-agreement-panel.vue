...
import type { MembershipAutoRenewAgreementPageRequest } from '#/api/types';

import { useVbenModal } from '@vben/common-ui';

import { formatQuery, useVbenVxeGrid } from '#/adapter/vxe-table';
import {
  membershipAutoRenewAgreementCancellationCreateApi,
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page, sorts }, formValues) => {
        return await membershipAutoRenewAgreementPageApi(
          formatQuery({
...
function buildAutoRenewAgreementSearchValues(
  ...
  } satisfies Partial<MembershipAutoRenewAgreementPageRequest>;
}
...
async function cancelAgreement(row: AutoRenewAgreementRow) {
  ...
  try {
    await membershipAutoRenewAgreementCancellationCreateApi(
      buildAutoRenewAgreementCancellationPayload(row),
    );
