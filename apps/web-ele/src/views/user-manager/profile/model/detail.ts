...
export function getDetailCards(detail: AdminAppUserDetailDto) {
  ...
  return [
    ...
    },
    {
      ...
      fields: [
        ...
        },
        {
          label: "获赞数",
          value:
            (counts?.forumTopicReceivedLikeCount ?? 0) + (counts?.commentReceivedLikeCount ?? 0),
          type: "text" as const,
        },
        {
          label: "获收藏数",
    ...
    {
      title: "管控信息",
      show: detail.status !== normalUserStatus || !!detail.banReason || !!detail.banUntil,
      fields: [
        {
          label: "当前状态",
      ...
      fields: [
        {
          label: "当前等级所需经验",
          value:
            experienceStats?.level?.requiredExperience ?? detail.level?.requiredExperience ?? "-",
          type: "text" as const,
        },
        {
          label: "下一等级所需经验",
