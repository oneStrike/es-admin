...
- 接入上游 Vben 组件或新增业务 wrapper / adapter 时，至少运行 `pnpm --filter @vben/web-ele typecheck` 或更高层级的 `pnpm check:type`，并确认改动没有越过 `apps/web-ele/src/**` 与本次允许的规范文档范围。
- 涉及表单 schema、表格操作、详情展示、数组字段或远程选项请求守卫时，应补充检查对应调用点是否仍保留原业务 API、payload 白名单和确认交互规则。
- 修改 `.trae/rules/*.md` 后，应对实际变更的规则文件执行 `pnpm exec oxfmt --check <changed-rule-files>`。
