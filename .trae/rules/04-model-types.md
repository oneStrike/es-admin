...
- 业务代码默认不手动声明领域类型；能由 TypeScript 推导、从生成 API 类型派生或由同域 helper 泛型约束的类型，不要额外显式声明。
...
- 函数返回类型默认交给 TypeScript 推导，除非公共 API 边界、递归结构或泛型约束确实需要，不手动声明返回类型。
...
- 业务代码应从生成类型中选取、派生、约束，而不是手动声明、复制 DTO 字段或放宽成 `any`。
